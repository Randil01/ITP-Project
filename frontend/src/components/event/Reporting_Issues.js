...
const ReportingIssues = () => {
  ...
  return (
    <div className="ebreporting-issues">
      <h2>Reporting Issues</h2>
      {error && <p className="eberror">{error}</p>}
      ...
      <button style={{ marginTop: '20px', padding: '10px 20px', fontSize: '16px', borderRadius: '5px', backgroundColor: '#008CBA', color: 'white', border: 'none', cursor: 'pointer' }} onClick={handlePrint}>Print Reports</button>
          ...
          {filteredReports.map((report, index) => (
            <tr key={report._id}>
              ...
              <td>
                ...
                <button onClick={() => handleDelete(index, report._id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
